...
var redisSentinelServers = []string{
	"tcp://127.0.0.1:26379",
}
...
func TestLockContextSentinel(t *testing.T) {
	...
	clis := make([]*redis.Client, 0, len(redisSentinelServers))
	for _, server := range redisSentinelServers {
		opts, err := parseSentinelConnString(server)
		assert.Nil(t, err)
		clis = append(clis, redis.NewFailoverClient(opts))
	}
	var wg sync.WaitGroup
	for _, cli := range clis {
		wg.Add(1)
