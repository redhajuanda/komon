...
import (
	...
	"github.com/redhajuanda/komon/lock/pkg/redlock"
)
...
func init() {
	...
	lock.Register(schemaRedisSentinel, NewSentinel)
}
...
func NewSentinel(urls []*url.URL) (lock.DLocker, error) {
	hs := make([]string, 0)
	for _, u := range urls {
		host := "tcp://"
	...
	lockMgr, err := redlock.NewRedLockSentinel(hs)
	if err != nil {
		return nil, err
	}
