...
func NewCache(url *url.URL) (cache.Cache, error) {
	...
	if d := url.Query().Get("enable_ttl"); d != "" {
		enable, _ := strconv.ParseBool(d)
		if enable {
			ttl(m)
		}
	}
...
// deleter check and delete expired data in memory cache
func ttl(m *MemoryCache) {
	go func() {
		for now := range time.Tick(5 * time.Second) {
			m.mux.Lock()
			for k, v := range m.data {
				if !v.expired.IsZero() && now.After(v.expired) {
					delete(m.data, k)
				}
			}
			m.mux.Unlock()
...
func (m *MemoryCache) set(key string, value interface{}, exp time.Duration) {
	m.mux.Lock()
	mo := memObject{value: value}
	if exp > 0 {
		mo.expired = time.Now().Add(exp)
	}
	m.data[key] = mo
	m.mux.Unlock()
}
...
func (m *MemoryCache) getWithoutDel(key string) interface{} {
	m.mux.RLock()
	val, ok := m.data[key]
	m.mux.RUnlock()
	if !ok {
...
// Exist check if key exist
func (m *MemoryCache) ExistWithoutDel(ctx context.Context, key string) bool {
	return m.getWithoutDel(key) != nil
}
